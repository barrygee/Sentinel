...
| **LAND**  | Three layers, one at a time: **APRS stations** from the [Direwolf sidecar](#aprs-decoding); **live traffic cameras** (Durham CC, TfL JamCams, UTMC, TrafficWatchNI — credentials in Settings › LAND › Live Camera Feeds); the **UK repeater directory** from [ukrepeater.net](https://ukrepeater.net/) with per-channel frequencies (tap to tune the SDR), modes, tones, BAND/MODE/STATUS filters and configurable labels. The register refreshes daily and falls back to `backend/data/uk_repeaters.json` offline; the directory is viewable/editable as JSON in Settings › LAND. |
...
Each decoder is a separate container behind its own compose **profile** — never built by default or in CI. They can all run at once, but **each needs its own dongle**: Sentinel takes an enforced lease on the receiver it is using. No configuration is needed; the ingest secret is auto-generated by the backend and shared via a Docker volume. A `401` in a sidecar's logs means it started before `app` wrote the secret — `docker compose --profile <name> restart <service>`.

```bash
docker compose --profile all up -d --build         # ALL decoders in one command
docker compose --profile decoder up -d --build     # digital voice (dsd-fme)
docker compose --profile aprs up -d --build        # APRS (Direwolf)
docker compose --profile ais up -d --build         # Off Grid AIS (Direwolf)
docker compose --profile adsb up -d --build        # Off Grid ADS-B (readsb)
docker compose --profile decoder --profile aprs up -d --build   # or combine profiles freely
docker compose --profile <name> logs -f <service>  # decoder · aprs-decoder · ais-decoder · adsb-decoder
docker compose --profile <name> down && docker compose up -d    # back to app-only
```

To make a bare `docker compose up` start them without the flag, set
`COMPOSE_PROFILES=all` in your `.env` (see `.env.example`) — it applies to every
compose command in the project, including `down` and `logs`.
...
Decodes ship AIS off the air with **Direwolf**, so the Sea map works with no internet connection — the off-grid alternative to AISStream.io. Vessels land in the same store as the online feed, so the map, tracks, ship-type icons and warm-start cache behave identically either way.
...
1. **Settings › SEA › AIS › Off Grid AIS SDR** — pick the radio on your marine antenna.
...
Needs a VHF marine antenna and line of sight — AIS is line-of-sight, so height beats gain. Note this container **builds Direwolf from source**: the AIS modem only works correctly from 1.8 onward, and Debian's 1.6 package decodes nothing. Details: [`decoder/ais/README.md`](decoder/ais/README.md).
