...
Sentinel is a real-time, multi-domain surveillance dashboard: aircraft (ADS-B), satellites (SGP4), ships (AIS), land (APRS, UK repeaters) and the radio spectrum (RTL-SDR) on one interactive map. A **FastAPI** backend serves a **Vue 3** single-page app rendering on **MapLibre GL**.
