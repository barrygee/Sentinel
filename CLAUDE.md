...
**Domains.** `air` (ADS-B), `space` (SGP4 + day/night terminator), `sea` (AIS via AISStream), `land` (APRS via Direwolf), `sdr` (RTL-SDR spectrum) are implemented. vue-router gates routes by `appStore.enabledDomains`.
