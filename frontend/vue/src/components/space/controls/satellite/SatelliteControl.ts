...
export class SatelliteControl extends SentinelControlBase {
  ...
  protected onInit(): void {
    ...
    this._onShowTracksChanged = (e: Event) => {
      ...
      ;['iss-track-orbit0', 'iss-track-orbit1', 'iss-track-orbit2', 'iss-track-orbit3'].forEach(
        (id) => {
          try {
  ...
  initLayers(): void {
    ;[
      ...
      'iss-track-orbit2',
      'iss-track-orbit3',
      'iss-footprint-fill-outer',
      'iss-footprint-fill-mid',
      'iss-footprint-fill-inner',
      'iss-footprint-fill-core',
      'iss-footprint-fill',
      'iss-footprint',
      'iss-bracket',
      'iss-icon',
    ...
    this.map.on('mouseenter', 'iss-icon', () => {
      this.map.getCanvas().style.cursor = 'pointer'
    })
    this.map.on('mouseenter', 'iss-bracket', () => {
      this.map.getCanvas().style.cursor = 'pointer'
    })
    this.map.on('mouseleave', 'iss-icon', () => {
      this.map.getCanvas().style.cursor = ''
    })
    this.map.on('mouseleave', 'iss-bracket', () => {
      this.map.getCanvas().style.cursor = ''
    })
  ...
  private _showNoTleOverlay(): void {
    document.getElementById('no-tle-overlay')?.classList.remove('hidden')
  }
  private _hideNoTleOverlay(): void {
    document.getElementById('no-tle-overlay')?.classList.add('hidden')
  }
  ...
  async _fetch(): Promise<void> {
    try {
      ...
      if (!this._previewNoradId) {
        ...
        const trackSource = this.map?.getSource('iss-track-source') as
          | maplibregl.GeoJSONSource
          | undefined
        const fpSource = this.map?.getSource('iss-footprint-source') as
          | maplibregl.GeoJSONSource
          | undefined
      ...
      document.dispatchEvent(
        new CustomEvent('iss-position-update', {
          detail: { ...position, noradId: this._activeNoradId },
        }),
      )
    } catch {}
  }
  ...
  get followedNoradId(): string | null {
    return this._followEnabled ? this._activeNoradId : null
  }

  get passNotificationsEnabled(): boolean {
    return this._passNotifier.enabled
  }
  get activeNoradId(): string {
    return this._activeNoradId
  }
  ...
  private _startFollowing(_restoring = false): void {
    ...
    document.dispatchEvent(
      new CustomEvent('satellite-follow-changed', {
        detail: { noradId: this._activeNoradId, following: true },
      }),
    )
  }
  ...
  private _stopFollowing(): void {
    ...
    document.dispatchEvent(
      new CustomEvent('satellite-follow-changed', {
        detail: { noradId: this._activeNoradId, following: false },
      }),
    )
  }
  ...
  private _hideStatusBar(): void {
    this._trackingStore.unregister('space')
  }
  ...
  async previewSatellite(noradId: string, name?: string): Promise<void> {
    ...
    try {
      const endpoint =
        (noradId === '25544' ? '/api/space/iss' : `/api/space/satellite/${noradId}`) +
        this._obsQuery()
      ...
      try {
        ...
        ;['iss-track-orbit0', 'iss-track-orbit1', 'iss-track-orbit2', 'iss-track-orbit3'].forEach(
          (id) => this.map.setLayoutProperty(id, 'visibility', trackVis),
        )
  ...
  switchSatellite(noradId: string, name: string, follow = false): void {
    ...
    if (!this.issVisible) {
      ...
      ;['iss-track-orbit0', 'iss-track-orbit1', 'iss-track-orbit2', 'iss-track-orbit3'].forEach(
        (id) => {
          try {
            this.map.setLayoutProperty(id, 'visibility', this.trackVisible ? 'visible' : 'none')
          } catch {}
        },
      )
      ;['iss-footprint-fill', 'iss-footprint'].forEach((id) => {
        try {
          this.map.setLayoutProperty(id, 'visibility', this.footprintVisible ? 'visible' : 'none')
        } catch {}
      })
      this.setButtonActive(true)
      this._spaceStore.setOverlay('iss', true)
