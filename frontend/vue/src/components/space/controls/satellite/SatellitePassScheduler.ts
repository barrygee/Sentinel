...
class ActionTrack {
  ...
  protected lateFireUntil(pass: IssPass): number {
    return pass.aos_unix_ms
  }
...
class AutoTuneTrack extends ActionTrack {
  ...
  constructor(private _ctx: SatellitePassSchedulerCtx) {
    super(
      0,
      () => _ctx.autoTuneEnabled(),
      (pass) => this._fireTune(pass),
    )
  }
  ...
  protected override lateFireUntil(pass: IssPass): number {
    return pass.los_unix_ms
  }
