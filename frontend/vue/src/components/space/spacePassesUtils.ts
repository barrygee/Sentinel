...
import { SATELLITE_CATEGORY_SHORT_LABELS, formatPassCountdown } from '../../utils/satelliteUtils'
...
export function passSecondary(pass: SatPass): string {
  const cat = pass.category
    ? SATELLITE_CATEGORY_SHORT_LABELS[pass.category] || pass.category.toUpperCase()
    : ''
...
export function findAutoTuneConflicts(
  ...
  for (const other of passes) {
    ...
    const overlaps = mine.some(
      (m) => m.aos_unix_ms < other.los_unix_ms && other.aos_unix_ms < m.los_unix_ms,
    )
