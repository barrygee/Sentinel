<template>
  <div id="air-playback-panel">
    <div class="sdr-group-body sdr-group-body-expanded">
      ...
      <div
        class="sdr-radio-section apb-section--start-date"
        :class="{ 'apb-section--locked': isActive }"
      >
        <!-- Start date picker -->
        <label class="sdr-field-label">START DATE</label>
        <div ref="calWrap" class="apb-cal-wrap">
          <button
            class="apb-date-btn"
            :class="{ 'apb-date-btn--chosen': !!pendingDate }"
            type="button"
            :disabled="isActive"
            @click="!isActive && (calOpen = !calOpen)"
          >
          ...
          <div v-if="calOpen && !isActive" class="apb-cal-popup" @mousedown.stop>
            <div class="apb-cal-header">
              <button class="apb-cal-nav" type="button" @click="calPrevMonth">&#8249;</button>
              <span class="apb-cal-month-label"
                >{{ CAL_MONTHS[calViewMonth] }} {{ calViewYear }}</span
              >
              <button class="apb-cal-nav" type="button" @click="calNextMonth">&#8250;</button>
            </div>
      ...
      <div
        class="sdr-radio-section sdr-radio-section--tight"
        :class="{ 'apb-section--locked': isActive }"
      >
        <!-- Start time -->
        <label class="sdr-field-label">START TIME</label>
        <div class="apb-time-row">
          <div ref="startHHWrap" class="apb-dd-wrap">
            <button
              class="apb-time-select"
              :class="{ 'apb-time-select--chosen': startHH !== '' }"
              type="button"
              :disabled="isActive"
              @click="!isActive && toggleDd('startHH')"
            >
            ...
            <Teleport v-if="openDd === 'startHH' && ddRect" to="body">
              <div
                class="apb-dd-list apb-dd-list--fixed"
                :style="{
                  top: ddRect.top + 'px',
                  left: ddRect.left + 'px',
                  width: ddRect.width + 'px',
                }"
                @mousedown.stop
              >
                <button
                  ...
                  class="apb-dd-item"
                  :class="{
                    'apb-dd-item--sel': startHH === h - 1,
                    'apb-dd-item--disabled': !isHourAvailable(h - 1),
                  }"
                  :disabled="!isHourAvailable(h - 1)"
                  type="button"
                  @click="selectStartHour(h - 1)"
                >
          ...
          <div ref="startMMWrap" class="apb-dd-wrap">
            <button
              class="apb-time-select"
              :class="{ 'apb-time-select--chosen': startMM !== '' }"
              type="button"
              :disabled="isActive"
              @click="!isActive && toggleDd('startMM')"
            >
            ...
            <Teleport v-if="openDd === 'startMM' && ddRect" to="body">
              ...
              >
                <button
                  ...
                  class="apb-dd-item"
                  :class="{
                    'apb-dd-item--sel': startMM === m - 1,
                    'apb-dd-item--disabled':
                      typeof startHH === 'number' &&
                      !availableMinutesFor(startHH as number).has(m - 1),
                  }"
                  :disabled="
                    typeof startHH === 'number' &&
                    !availableMinutesFor(startHH as number).has(m - 1)
      ...
      <div class="sdr-radio-section" :class="{ 'apb-section--locked': isActive }">
        ...
        <div ref="endCalWrap" class="apb-cal-wrap">
          <button
            class="apb-date-btn"
            :class="{ 'apb-date-btn--chosen': !!pendingEndDate }"
            type="button"
            :disabled="isActive"
            @click="!isActive && (endCalOpen = !endCalOpen)"
          >
          ...
          <div v-if="endCalOpen && !isActive" class="apb-cal-popup" @mousedown.stop>
            <div class="apb-cal-header">
              <button class="apb-cal-nav" type="button" @click="endCalPrevMonth">&#8249;</button>
              <span class="apb-cal-month-label"
                >{{ CAL_MONTHS[endCalViewMonth] }} {{ endCalViewYear }}</span
              >
              <button class="apb-cal-nav" type="button" @click="endCalNextMonth">&#8250;</button>
            </div>
            <div class="apb-cal-grid">
              <span
                v-for="d in ['M', 'T', 'W', 'T', 'F', 'S', 'S']"
      ...
      >
        ...
        <div class="apb-time-row">
          <div ref="endHHWrap" class="apb-dd-wrap">
            <button
              class="apb-time-select"
              :class="{ 'apb-time-select--chosen': endHH !== '' }"
              type="button"
              :disabled="isActive"
              @click="!isActive && toggleDd('endHH')"
            >
            ...
            <Teleport v-if="openDd === 'endHH' && ddRect" to="body">
              ...
              >
                <button
                  ...
                  class="apb-dd-item"
                  :class="{
                    'apb-dd-item--sel': endHH === h - 1,
                    'apb-dd-item--disabled':
                      !isHourAvailable(h - 1) ||
                      (pendingEndDate === pendingDate &&
                        typeof startHH === 'number' &&
                        h - 1 < startHH),
                  }"
                  :disabled="
          ...
          <span class="apb-time-colon">:</span>
          <div ref="endMMWrap" class="apb-dd-wrap">
            <button
              class="apb-time-select"
              :class="{ 'apb-time-select--chosen': endMM !== '' }"
              type="button"
              :disabled="isActive"
              @click="!isActive && toggleDd('endMM')"
            >
            ...
            <Teleport v-if="openDd === 'endMM' && ddRect" to="body">
              ...
              >
                <button
                  ...
                  class="apb-dd-item"
                  :class="{
                    'apb-dd-item--sel': endMM === m - 1,
                    'apb-dd-item--disabled':
                      (typeof endHH === 'number' &&
                        !availableMinutesFor(endHH as number).has(m - 1)) ||
                      (pendingEndDate === pendingDate &&
                        typeof endHH === 'number' &&
                        typeof startHH === 'number' &&
                        endHH === startHH &&
                        typeof startMM === 'number' &&
                        m - 1 <= startMM),
                  }"
                  :disabled="
                    (typeof endHH === 'number' &&
      ...
      <!-- ── Timeline canvas ── -->
      <div
        class="sdr-radio-section sdr-radio-section--tight apb-section--timeline"
        :class="{ 'apb-section--locked': !isActive }"
      >
        <div ref="timelineWrap" class="apb-timeline-wrap">
          <canvas
            ref="timelineCanvas"
            class="apb-timeline-canvas"
...
function isHourAvailable(h: number): boolean {
  return availableHours.value.has(h)
}
...
// Time-picker selection handlers. Extracted from inline multi-statement template
// handlers so they survive Prettier formatting (no-semicolon style breaks
// multi-statement inline @click expressions in the Vue template compiler).
...
function endCalPrevMonth(): void {
  ...
  endCalViewMonth.value = next.month
...
function loadPlayback(): void {
  ...
  const eh = endHH.value as number,
    em = endMM.value as number
  const startMs = new Date(
    `${pendingDate.value}T${String(sh).padStart(2, '0')}:${String(sm).padStart(2, '0')}:00Z`,
  ).getTime()
  const endMs = new Date(
    `${pendingEndDate.value}T${String(eh).padStart(2, '0')}:${String(em).padStart(2, '0')}:00Z`,
  ).getTime()
...
watch(
  [() => playbackStore.cursorMs, () => playbackStore.status, () => playbackStore.windowStartMs],
  () => {
