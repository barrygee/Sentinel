    ...
    process(list: ParsedAircraft[]): void {
        ...
        for (const a of list) {
            ...
            if (justDeparted) {
                ...
                this._notifications.add({ type: 'departure', title: callsign, ...(apt ? { detail: `${apt.name} (${apt.icao})` } : {}) })
            }

            if (justLanded && optedIn) {
                const callsign = a.flight || a.r.trim() || this._notif.callsignFor(hex)
                const apt = this._nearestAirport(a.lat, a.lon)
                this._notifications.add({ type: 'flight', title: callsign, ...(apt ? { detail: `${apt.name} (${apt.icao})` } : {}) })
            }
        }

        // Prune transition state for aircraft no longer in the feed.
        for (const hex of Object.keys(this._prevAlt))    { if (!seen.has(hex)) delete this._prevAlt[hex] }
        for (const hex of Object.keys(this._hasDeparted)){ if (!seen.has(hex)) delete this._hasDeparted[hex] }
        for (const hex of Object.keys(this._seenOnGround)){ if (!seen.has(hex)) delete this._seenOnGround[hex] }
    }
