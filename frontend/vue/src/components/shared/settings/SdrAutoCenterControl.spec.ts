...
describe('SdrAutoCenterControl', () => {
  ...
  it('renders the switch reflecting the store default (on)', async () => {
    const wrapper = mount(SdrAutoCenterControl)
    await flushPromises()
  ...
  it('toggles off, mirrors into the store, and stages the DB write', async () => {
    ...
    expect(sdr.autoCenterWaterfallOnTune).toBe(false)
  ...
  it('hydrates the toggle from the DB on mount', async () => {
    stubFetch({ autoCenterWaterfallOnTune: false })
    const wrapper = mount(SdrAutoCenterControl)
    await flushPromises()
    expect(useSdrStore().autoCenterWaterfallOnTune).toBe(false)
    expect(wrapper.find('[role="switch"]').attributes('aria-checked')).toBe('false')
  })
