<template>
  <div class="ac-wrap">
    <div class="ac-row">
      <span class="ac-label">NOTIFICATION SOUND</span>
      <button
        class="ac-track"
        :class="{ 'is-on': on }"
        role="switch"
...
const on = ref<boolean>(app.notificationSound)

async function syncFromDb(): Promise<void> {
  await app.hydrateNotificationSoundFromDb()
  on.value = app.notificationSound
}
...
function toggle(): void {
  on.value = !on.value
  app.setNotificationSound(on.value)
  emit('stage', () => settingsApi.put('app', 'notificationSound', on.value))
}
</script>
