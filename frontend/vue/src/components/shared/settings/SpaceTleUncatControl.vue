<template>
  <div class="tle-uncat-wrap">
    ...
    <div v-if="satellites.length > 0" class="tle-uncat-list">
      <div v-for="sat in satellites" :key="sat.norad_id" class="tle-uncat-row">
        <span class="tle-uncat-name">{{ sat.name }}</span>
        <span class="tle-uncat-id">{{ sat.norad_id }}</span>
        <div
          class="tle-dropdown tle-uncat-drop"
          :class="{ 'tle-dropdown--open': openDrop === sat.norad_id }"
          tabindex="0"
          @blur="openDrop = ''"
        >
          <div
            class="tle-dropdown-selected"
            @mousedown.prevent="openDrop = openDrop === sat.norad_id ? '' : sat.norad_id"
          >
            <span
              class="tle-dropdown-selected-text"
              :class="{ 'tle-dropdown-selected-text--chosen': assignments[sat.norad_id] }"
            >
              {{
                assignments[sat.norad_id]
                  ? catLabel(assignments[sat.norad_id])
                  : TLE_ASSIGN_CATEGORIES[0].label
              }}
            </span>
            <span class="tle-dropdown-arrow"></span>
          </div>
          <div
            class="tle-dropdown-menu"
            :class="{ 'tle-dropdown-menu--open': openDrop === sat.norad_id }"
          >
            <div
...
async function load(): Promise<void> {
  try {
    ...
    countLine.value = 'Failed to load'
  }
}
