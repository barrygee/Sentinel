...
(function buildSdrTab() {
    ...
    function _wireControls() {
        // ── Element refs ─────────────────────────────────────────────────────
        const dotEl = document.getElementById('sdr-tab-dot');
        const freqInput = document.getElementById('sdr-tab-freq');
        const modePillsEl = document.getElementById('sdr-tab-mode-pills');
        const signalBar = document.getElementById('sdr-tab-signal-bar');
        const volSlider = document.getElementById('sdr-tab-vol');
        const volVal = document.getElementById('sdr-tab-vol-val');
        const sqSlider = document.getElementById('sdr-tab-sq');
        const sqVal = document.getElementById('sdr-tab-sq-val');
        const playBtn = document.getElementById('sdr-tab-play');
        const stopBtn = document.getElementById('sdr-tab-stop');
        const recBtn = document.getElementById('sdr-tab-rec');
        const recTimer = document.getElementById('sdr-tab-rec-timer');
        const radioSelect = document.getElementById('sdr-tab-radio');
        const bwSlider = document.getElementById('sdr-tab-bw');
        const bwVal = document.getElementById('sdr-tab-bw-val');
        const gainSlider = document.getElementById('sdr-tab-gain');
        const gainVal = document.getElementById('sdr-tab-gain-val');
        const agcCheck = document.getElementById('sdr-tab-agc');
        const deviceDropdown = document.getElementById('sdr-tab-device-dropdown');
        const deviceText = document.getElementById('sdr-tab-device-text');
        const clipsCount = document.getElementById('sdr-tab-clips-count');
        const clipsList = document.getElementById('sdr-tab-clips-list');
        const clipsEmpty = document.getElementById('sdr-tab-clips-empty');
        const recModal = document.getElementById('sdr-tab-rec-modal');
        const recmodName = document.getElementById('sdr-tab-recmod-name');
        const recmodNotes = document.getElementById('sdr-tab-recmod-notes');
        const recmodCancel = document.getElementById('sdr-tab-recmod-cancel');
        const recmodSave = document.getElementById('sdr-tab-recmod-save');
        const recDelModal = document.getElementById('sdr-tab-rec-del-modal');
        const recDelMsg = document.getElementById('sdr-tab-recdelmod-msg');
        const recDelCancel = document.getElementById('sdr-tab-recdelmod-cancel');
        const recDelConfirm = document.getElementById('sdr-tab-recdelmod-confirm');
        ...
        function renderClips() {
            ...
            visible.forEach((c) => {
                ...
                const playBtn2 = row.querySelector('.sdr-clip-play-btn');
                const audio = document.getElementById(`sdr-tab-clip-audio-${c.id}`);
                const player = document.getElementById(`sdr-tab-clip-player-${c.id}`);
                const seekEl = document.getElementById(`sdr-tab-clip-seek-${c.id}`);
                const curEl = document.getElementById(`sdr-tab-clip-cur-${c.id}`);
                const durEl = document.getElementById(`sdr-tab-clip-dur-${c.id}`);
                const playIcon = playBtn2.querySelector('.sdr-clip-play-icon');
                const stopIcon2 = playBtn2.querySelector('.sdr-clip-stop-icon');
                function fmtT(s) {
                    const m = Math.floor(s / 60), sec = Math.floor(s % 60);
                    return String(m).padStart(2, '0') + ':' + String(sec).padStart(2, '0');
                }
                function setClipPlaying(on) {
        ...
        async function reloadClips() {
            try {
                ...
                console.error('[SDR-TAB] reloadClips failed:', e);
            }
            renderClips();
        }
        ...
        window._SdrMiniPlayer = { tune, show, hide, updateSignalBar, setConnected, populateRadios, onSquelchChange };
    }
