...
window._SpacePassesPanel = (() => {
    ...
    let _expandedNoradId: string | null = null;

    // Per-item state for sat info fetch / tick
    let _itemFetchAbort: AbortController | null = null;
    let _itemTickInterval: ReturnType<typeof setInterval> | null = null;
    ...
    let _listTickInterval: ReturnType<typeof setInterval> | null = null;
    ...
    // Sat-info passes API response
    interface SatInfoPass {
        aos_utc:           string;
        los_utc:           string;
        aos_unix_ms:       number;
        los_unix_ms:       number;
    ...
    function _clearItemState(): void {
        if (_itemFetchAbort) { _itemFetchAbort.abort(); _itemFetchAbort = null; }
        if (_itemTickInterval) { clearInterval(_itemTickInterval); _itemTickInterval = null; }
    }
    ...
    function _collapseExpanded(): void {
        const list = document.getElementById('spp-list');
        if (!list) return;
        const expanded = list.querySelector('.spp-pass-card.spp-expanded') as HTMLElement | null;
        if (expanded) {
            expanded.classList.remove('spp-expanded');
            const body = expanded.querySelector('.spp-acc-body') as HTMLElement | null;
            if (body) body.remove();
        }
    ...
    function _buildPassAccordionBody(noradId: string, name: string): HTMLElement {
        ...
        trackBtn.addEventListener('click', (e) => {
            e.stopPropagation();
            if (issControl) issControl.switchSatellite(noradId, name);
            close();
        });
    ...
    function _renderSatPassesInAccordion(satPassList: HTMLElement, statusEl: HTMLElement, passes: SatInfoPass[], computedAt: string): void {
        const computedDate = new Date(computedAt);
    ...
    function _startItemTick(satPassList: HTMLElement): void {
        if (_itemTickInterval) clearInterval(_itemTickInterval);
        _itemTickInterval = setInterval(() => {
            const now = Date.now();
            satPassList.querySelectorAll<HTMLElement>('.spp-acc-pass-card').forEach(el => {
                const aosMs = parseInt(el.dataset['aosMs'] || '0', 10);
                const losMs = parseInt(el.dataset['losMs'] || '0', 10);
                const cd = el.querySelector('.spp-acc-pass-countdown') as HTMLElement | null;
                if (!cd) return;
    ...
    async function _fetchAndPopulateAccordion(noradId: string, body: HTMLElement): Promise<void> {
        _clearItemState();
        _itemFetchAbort = new AbortController();
        const abort = _itemFetchAbort;

        const satPassList = body.querySelector('.spp-acc-pass-list') as HTMLElement | null;
        const statusEl = body.querySelector('.spp-acc-status') as HTMLElement | null;

        if (!satPassList || !statusEl) return;
        ...
        try {
            ...
            const resp = await fetch(url, { signal: abort.signal });
            if (abort.signal.aborted) return;
            if (!resp.ok) { statusEl.textContent = 'COULD NOT LOAD PASSES'; statusEl.classList.remove('spp-acc-status-loading'); return; }
            const data = await resp.json() as SatInfoApiResponse;
    ...
    function updateExpandedPosition(p: { alt_km: number; velocity_kms: number; track_deg: number; lat: number; lon: number }): void {
    ...
    function _startListCountdownTick(): void {
        if (_listTickInterval) clearInterval(_listTickInterval);
    ...
    function open(): void {
        ...
        if (!_listTickInterval) _startListCountdownTick();
    }
    ...
    function init(): void {
        ...
        document.addEventListener('click', (e: MouseEvent) => {
            ...
            if (targetElement && targetElement.closest('.msb-tab') && (targetElement.closest('.msb-tab') as HTMLElement).dataset['tab'] === 'passes') {
                if (_injected) void _fetchPasses();
            }
        });

        // Forward live telemetry into the expanded pass accordion
        document.addEventListener('sat-position-update', (e: Event) => {
            if (!_expandedNoradId) return;
            const { noradId, position } = (e as CustomEvent<{ noradId: string; position: { alt_km: number; velocity_kms: number; track_deg: number; lat: number; lon: number } }>).detail;
            if (noradId !== _expandedNoradId) return;
