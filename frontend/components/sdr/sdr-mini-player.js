...
(function buildSdrMiniPlayer() {
    ...
    const modeBtn = document.getElementById('sdr-mini-mode-btn');
    ...
    // ── Mode button ───────────────────────────────────────────────────────────
    function currentMode() { return MODES[_modeIndex]; }
    ...
    function play() {
        ...
        if (!_sdrCurrentRadioId) {
            ...
            if (savedId > 0)
                _sdrCurrentRadioId = savedId;
        }
        setPlaying(true);
        if (!_sdrCurrentRadioId)
            return; // no radio configured — UI armed, deferred to populateRadios
        const mode = currentMode();
        if (window._SdrAudio) {
    ...
    function show() {
        ...
        try {
            sessionStorage.setItem('sdrMiniVisible', '1');
        }
        catch (_e) { }
    }
    function hide() {
        ...
        try {
            sessionStorage.removeItem('sdrMiniVisible');
        }
        catch (_e) { }
    }
    function toggle() {
    ...
    window._SdrMiniPlayer = { tune, show, hide, toggle, updateSignalBar, setConnected, populateRadios };
})();
